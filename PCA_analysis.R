...
###########################################
#    First step: make the fake sample     #
###########################################
## columns are individual samples (i.e. cells)                      
## rows are measurements taken for all the samples (i.e. genes)     
data.matrix <- matrix(nrow=100, ncol=10) ## make a matrix of 10 columns and 100 rows
colnames(data.matrix) <- c(
  paste("wt", 1:5, sep=""),
  paste("ko", 1:5, sep=""))  ## first 5 columns named wt1, wt2, etc; last 5 columns names ko1, ko2, etc.
rownames(data.matrix) <- paste("gene", 1:100, sep="") ## name as gene1, gene2, etc
for (i in 1:100) {
  wt.values <- rpois(5, lambda=sample(x=10:1000, size=1))
  ko.values <- rpois(5, lambda=sample(x=10:1000, size=1))
  data.matrix[i,] <- c(wt.values, ko.values)
} ## give value to each site
head(data.matrix)
dim(data.matrix)

#################################################
#   Second step: caculate the PCA by prcomp()   # 
#################################################
## prcomp() need row is the cells and columns are the gene -> use t() to change
## Result--3 important valus
  ## 1) 'x'-> the PC valus (PC1, PC2)
  ## 2) 'sdev'-> standard deviation ~ to calculate the proportion of certain PC in the variation
  ## 3) 'rotation'-> loading score ~ the gene's contribution
pca <- prcomp(t(data.matrix), scale=TRUE) 

#################################################################
#   Third step: plot pc1 and pc2 use 'x' (first edition plot)   #
#################################################################
plot(pca$x[,1], pca$x[,2])

################################################
#   Fourth step: make a scree plot use 'sdev'  #
################################################
pca.var <- pca$sdev^2  
pca.var.per <- round(pca.var/sum(pca.var)*100, 1) ## proportion
barplot(pca.var.per, main="Scree Plot", xlab="Principal Component", ylab="Percent Variation")

#################################################
#   Fifth step: make a better look plot of PC   #
#################################################
library(ggplot2)
...
ggplot(data=pca.data, aes(x=X, y=Y, label=Sample)) +
  geom_text() +## plot the name 
  xlab(paste("PC1 - ", pca.var.per[1], "%", sep="")) +
  ylab(paste("PC2 - ", pca.var.per[2], "%", sep="")) +## x, y have the percentage of PC1 & PC2 in variation
  theme_bw() +## make the background white
  ggtitle("My PCA Graph") ## add title

#######################################################
#   Sixth step: get the the top 10 contribute genes   #
#######################################################
## PC1 contribute most to vvariation ~ use PC1 data
loading_scores <- pca$rotation[,1]  ## PC1
gene_scores <- abs(loading_scores)  ## has + & -
